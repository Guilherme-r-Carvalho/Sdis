...
public class AddressBookServiceImpl extends AddressBookServiceImplBase {
    ...
    public void listAll(ListAllRequest request, StreamObserver<ListAllResponse>rObserver) {
        try {
            ListAllResponse response = ListAllResponse.newBuilder().addAllPeople(addressBook.listAll(request.getPhoneType())).build();
            rObserver.onNext(response);
            rObserver.onCompleted();
        }
        catch (IllegalArgumentException e) {
            rObserver.onError(INVALID_ARGUMENT.withDescription("\nNão existem contactos com o tipo dado.").asRuntimeException());
        }
    }
}
